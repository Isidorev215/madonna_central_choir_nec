  ...
  <div class="relative z-spinner" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
      <div @click.self="$emit('closeModal')" class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div class="grid-cols-5 space-y-2 lg:space-y-0 lg:grid lg:gap-3 lg:grid-rows-2 transform overflow-hidden rounded-lg text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl ">
          <UserDetailsCard class="w-full row-span-3 col-span-3" :user="user"/>
          <UserDateCard class="w-full col-span-2" :user="user"/>
          <UserStatusCard class="w-full col-span-2" :user="user"/>
...
import UserDateCard from '@/components/UserDateCard.vue'
