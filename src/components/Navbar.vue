    ...
    <div class="icons flex justify-end items-center space-x-2 lg:space-x-5">
      ...
      <div class="profile-drop h-full dropdown relative" ref="profileRoot">
        <button @click="toggleProfileDrop()" data-dropdown-toggle="dropdown" class="text-black relative font-medium lg:py-2 lg:px-3 lg:ring-1 ring-dark-gray-2 rounded-md text-sm text-center inline-flex items-center" type="button">
          ...
          <AvatarInitial :image="user?.profileImage" :name="`${user?.firstName} ${user?.lastName}`" :dimension="30" :rounded="9999" class="w-14 h-auto align-middle" />
          <span class="hidden lg:flex justify-center items-center font-medium text-base ml-2">{{`${user?.firstName} ${user?.lastName}`}}</span>
